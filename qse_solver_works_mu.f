		...
		enddo
		 ...
		 do i=1, mt% Ntable    
			...
			A(mt% Ntable+1, mt%Ntable+1) = -(real(mt%Z(1))/kT)+ &
			 & (real(mt%Z(1)-mt%Z(i))/kT)*sum_lnZ_total/(1.0+sum_lnZ_total)	&
			 & - 1.0/n_e/dmudne

     		A(mt%Ntable+1,mt%Ntable+2) = (real(mt%A(1))/kT)+ &
			 & (real(mt%A(i)-mt%A(1))/kT)*sum_lnZ_total/(1.0+sum_lnZ_total)	
			 
     		A(mt%Ntable+2, mt%Ntable+1) =-(real(mt%Z(1))/kT)+ &
			 & (real(mt%Z(1)-mt%Z(i))/kT)*sum_lnA_total/(1.0+sum_lnA_total)	
     				
     		A(mt% Ntable+2, mt%Ntable+2) = (real(mt%A(1))/kT)+ &
			 & (real(mt%A(i)-mt%A(1))/kT)*sum_lnA_total/(1.0+sum_lnA_total)	&
			 & + 1.0/(n_b-n_n)/(-dmudnn)  
